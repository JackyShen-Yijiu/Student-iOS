...
// 提交预支付
-(NSMutableDictionary *)sendPrepayWithParams:(NSMutableDictionary *)prePayParams
{
    //获取提交支付
...
// 负责人：李启波（marcyli）
//============================================================
- (NSMutableDictionary *)sendPay_demoWithtradeNO:(NSString *)tradeNO amount:(NSString *)amount productName:(NSString *)productName productDescription:(NSString *)productDescription type:(NSInteger)type
{
    
    //订单标题，展示给用户
    NSString *order_name    = [NSString stringWithFormat:@"%@",productName];
    //订单金额,单位（分）
    NSString *order_price   = [NSString stringWithFormat:@"%.0f",[amount floatValue] * 100];//1分钱测试
    NSLog(@"order_price:%@",order_price);
    
    NSString *trade_type = [NSString stringWithFormat:@"%@",(type == 1) ? @"APP" : @"NATIVE"];
    ...
    [packageParams setObject: trade_type            forKey:@"trade_type"];  //支付类型，固定为APP
