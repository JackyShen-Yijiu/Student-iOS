...
#pragma mark --------- FMDB保存数据
+ (void)setupUserData
{
    // 0.获得沙盒中的数据库文件名
    NSString *path = [YBPath stringByAppendingPathComponent:@"yibuxuecheUserData.sqlite"];
    ...
    [_queue inDatabase:^(FMDatabase *db) {
        ...
        BOOL result = [db executeUpdate:@"create table if not exists t_status (id integer primary key autoincrement, status blob);"];
...
+ (void)saveUserData:(YBUserData *)userData
{
    [self setupUserData];
...
+ (YBUserData *)getUserData
{
    ...
    [_queue inDatabase:^(FMDatabase *db) {
        ...
        while (rs.next) {
            ...
            YBUserData *status = [NSKeyedUnarchiver unarchiveObjectWithData:data];
